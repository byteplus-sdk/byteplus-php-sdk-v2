...
| 变量 | 说明 | 必填 |
|---|---|:-:|
| `BYTEPLUS_ACCESSKEY` / `BYTEPLUS_ACCESS_KEY` | Access Key（两种写法均可） | ✅ |
| `BYTEPLUS_SECRETKEY` / `BYTEPLUS_SECRET_KEY` | Secret Key（两种写法均可） | ✅ |
...
Access Key / Secret Key 支持「连写」与「下划线分隔」两种写法，两者同时设置时以连写形式优先：
