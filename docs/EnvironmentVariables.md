...
|---|---|:-:|
| `BYTEPLUS_ACCESSKEY` / `BYTEPLUS_ACCESS_KEY` | Access Key (either name is accepted) | ✅ |
| `BYTEPLUS_SECRETKEY` / `BYTEPLUS_SECRET_KEY` | Secret Key (either name is accepted) | ✅ |
...
For Access Key / Secret Key, both the concatenated and underscore-separated forms are accepted; the concatenated form takes priority when both are set:
