...
class OidcCredentialProvider extends Provider
{
    ...
    {
        $roleTrn = getenv('BYTEPLUS_OIDC_ROLE_TRN');
        $roleSessionName = getenv('BYTEPLUS_OIDC_ROLE_SESSION_NAME') ?: null;
        $oidcTokenFile = getenv('BYTEPLUS_OIDC_TOKEN_FILE');
        $rolePolicy = getenv('BYTEPLUS_OIDC_ROLE_POLICY') ?: null;
        $stsEndpoint = getenv('BYTEPLUS_OIDC_STS_ENDPOINT') ?: null;

        if (empty($roleTrn) || empty($oidcTokenFile)) {
            throw new ApiException(
                self::PROVIDER_NAME . ': required environment variables '
                . 'BYTEPLUS_OIDC_ROLE_TRN and '
                . 'BYTEPLUS_OIDC_TOKEN_FILE are not all set'
            );
        }
