...
class CLIConfigCredentialProvider extends Provider
{
    ...
    private function resolveConfigPath()
    {
        ...
        $envPath = getenv('BYTEPLUS_CLI_CONFIG_FILE');
        if ($envPath !== false && $envPath !== '') {
            return $envPath;
        }

        $home = $this->getHomeDir();
        return $home . DIRECTORY_SEPARATOR . '.byteplus' . DIRECTORY_SEPARATOR . 'config.json';
    }
    ...
    private function resolveProfile($config)
    {
        ...
        $envProfile = getenv('BYTEPLUS_PROFILE');
    ...
    {
        $envPath = getenv('BYTEPLUS_LOGIN_CACHE_DIRECTORY');
        if ($envPath !== false && $envPath !== '') {
            return $envPath;
        }
