...
assert_contains($errorHandling, 'Byteplus\\Common\\ApiException', 'error docs api exception');
...
assert_not_contains($signInterceptor, 'Byteplus\\Common\\Sign', 'sign interceptor sign namespace');
...
assert_contains($endpointStandard, 'SITE_STACK_BYTEPLUS_DUAL_STACK', 'standard endpoint dualstack site stack');
...
$credentials = read_file_or_fail($root . '/docs/1-Credentials.md');
assert_not_contains($credentials, '\\Byteplus\\Vpc\\API\\VPCApi', 'credentials docs VPC API namespace');
assert_contains($credentials, '\\Byteplus\\Vpc\\Api\\VPCApi', 'credentials docs VPC Api namespace');
...
assert_not_contains($credentialsZh, '\\Byteplus\\Vpc\\API\\VPCApi', 'credentials zh docs VPC API namespace');
assert_contains($credentialsZh, '\\Byteplus\\Vpc\\Api\\VPCApi', 'credentials zh docs VPC Api namespace');

$stsFormRequest = read_file_or_fail($root . '/src/Common/Auth/Providers/StsFormRequest.php');
assert_contains($stsFormRequest, 'doPostWithRetry', 'sts form retry helper');
assert_contains($stsFormRequest, 'HTTP 5xx / 429', 'sts form retryable status documentation');
